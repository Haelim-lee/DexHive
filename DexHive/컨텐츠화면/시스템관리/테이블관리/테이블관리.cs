...
{
    public partial class 테이블관리 : 스크린, I스크린
    {
        public 테이블관리표_모델 그리드모델 {get;set;}
        ...
        public void 행추가()
        {
            if (테이블관리표.DataSource == null) 조회();
            {
                ((DataTable)테이블관리표.DataSource).Rows.Add();
            }
        }

        public void 행제거()
        {
            if (테이블관리표.CurrentRow != null)
                if ((DataRowView)테이블관리표.CurrentRow.DataBoundItem != null)
                {
                    bool a = 테이블관리표.CurrentRow.IsNewRow;
                    DataRow DR = ((DataRowView)테이블관리표.CurrentRow.DataBoundItem).Row;
                    테이블관리표.Rows.Remove(테이블관리표.CurrentRow);
