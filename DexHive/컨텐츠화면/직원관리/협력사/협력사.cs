...
{
    public partial class 협력사 : 스크린, I스크린
    {
        public 협력사표_모델 그리드모델 {get;set;}
        ...
        public void 행추가()
        {
            if (협력사표.DataSource == null) 조회();
            {
                ((DataTable)협력사표.DataSource).Rows.Add();
            }
        }

        public void 행제거()
        {
            if (협력사표.CurrentRow != null)
                if ((DataRowView)협력사표.CurrentRow.DataBoundItem != null)
                {
                    bool a = 협력사표.CurrentRow.IsNewRow;
                    DataRow DR = ((DataRowView)협력사표.CurrentRow.DataBoundItem).Row;
                    협력사표.Rows.Remove(협력사표.CurrentRow);
