...
{
    public partial class 교육관리 : 스크린, I스크린
    {
        public 교육관리표_모델 그리드모델 { get; set; }
        ...
        public void 행추가()
        {
            if (교육관리표.DataSource == null) 조회();
            {
                ((DataTable)교육관리표.DataSource).Rows.Add();
            }
        }

        public void 행제거()
        {
            if (교육관리표.CurrentRow != null)
                if ((DataRowView)교육관리표.CurrentRow.DataBoundItem != null)
                {
                    bool a = 교육관리표.CurrentRow.IsNewRow;
                    DataRow DR = ((DataRowView)교육관리표.CurrentRow.DataBoundItem).Row;
                    교육관리표.Rows.Remove(교육관리표.CurrentRow);
