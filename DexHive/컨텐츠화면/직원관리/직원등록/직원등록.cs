...
{
    public partial class 직원등록 : 스크린, I스크린
    {
        public 직원등록표_모델 그리드모델 {get;set;}
        ...
        public void 행추가()
        {
            if (직원등록표.DataSource == null) 조회();
                ((DataTable)직원등록표.DataSource).Rows.Add();
        }

        public void 행제거()
        {
            if (직원등록표.CurrentRow != null)
                if ((DataRowView)직원등록표.CurrentRow.DataBoundItem != null)
                {
                    bool a = 직원등록표.CurrentRow.IsNewRow;
                    DataRow DR = ((DataRowView)직원등록표.CurrentRow.DataBoundItem).Row;
                    직원등록표.Rows.Remove(직원등록표.CurrentRow);
