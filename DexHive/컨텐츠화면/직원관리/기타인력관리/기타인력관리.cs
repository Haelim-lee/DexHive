...
{
    public partial class 기타인력관리 : 스크린, I스크린
    {
        public 기타인력관리표_모델 그리드모델 {get;set;}
        ...
        {
            그리드모델 = new 기타인력관리표_모델();
            그리드모델.그리드 = 기타인력관리표;
            그리드모델.그리드.기본테이블명 = "직원정보";
        }
        private void InitializeEvent()
        {
            this.Shown += 직원관리화면_Load;//직원관리화면
        ...
        public void 행추가()
        {
            if (기타인력관리표.DataSource == null) 조회();
            {
                ((DataTable)기타인력관리표.DataSource).Rows.Add();
            }
        }

        public void 행제거()
        {
            if (기타인력관리표.CurrentRow != null)
                if ((DataRowView)기타인력관리표.CurrentRow.DataBoundItem != null)
                {
                    bool a = 기타인력관리표.CurrentRow.IsNewRow;
                    DataRow DR = ((DataRowView)기타인력관리표.CurrentRow.DataBoundItem).Row;
                    기타인력관리표.Rows.Remove(기타인력관리표.CurrentRow);
